...
double qbremss(double tempe)
{
    ...
    qei=1.48e-22*eDensity*iDensity*fei; //Eq 3.5 ne² ? no deberia ser eDensity*eDensity
...
double qiefunc(double tempi, double tempe, double mdot)  //Eq 3.1 o 3.3
{
    extern double r;
    double rad=r;
...
double qem(double tempe, double mdot) 
{
    ...
    double aux=4.0*SIGMASB*pow(ELECTRONMASS*CLIGHT2*tempe/BOLTZMANN,4)/height();
