...
void constants2()
{
    ...
    mdot=1.0e-3;                                // [Eddington luminosities]
    alphapar=0.03;
...
double massdens(double f) {
    extern double r,m,alphapar,mdot;
...
double taues(double f) {
    extern double r,alphapar,mdot;
    return 12.4*mdot/(alphapar*c1(f)*sqrt(r));
}

double height(double f) {
    extern double radius;
    return radius*sqrt(2.5*c3(f));
}