...
{ 
	...
	double factor  = 1-xalpha;

	double f;
	if (factor > 0)
		f = 4*alpha*Bpi * xalpha/x * pow(factor/(1+r*xalpha*factor),4) * 
				(1.0/factor + r*(1-2*xalpha)/(1+r*xalpha*factor))*sqrt(1-chargedPionMass*cLight2/(x*Ep));
	else
		f = 0.0;
