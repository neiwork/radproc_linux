...
#include "gaussianScaledPivoting.h"
...
void gaussianScaledPivoting(int N, Matrix& A, Vector& X)
{
	...
	//int NROW[10];  //este creo que es el que reordena; ver la dimension ser� Nrow[X.size()] ?
...
//	int NROW[X.size()];
	...
	if (OK) {
		...
		if (OK) {
			/* STEP 9 */
			N1 = NROW[N - 1];
			if (abs(A[N1 - 1][N - 1]) <= ZERO) OK = false;
			/* system has no unique solution */
			else {
				/* STEP 10 */
...
void INPUT(int *OK, Matrix& A, int *N)
{
	int I, J, FLAG;
	char AA;
	char NAME[30];
