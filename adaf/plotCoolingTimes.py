...
r,log_ge,logAcce,logAdve,logDiffKe,logDiffBe,logDiffPare,logDiffPere,logEmaxHe,logSye,logIC,logBr,logeRelax = np.loadtxt('electronCoolingTimes.dat',unpack=True,skiprows=1)
r,log_gp,logAccp,logAdvp,logDiffKp,logDiffBp,logDiffParp,logDiffPerp,logEmaxHp,logSyp,logICp,logpp,logpg,logBH,logAdi,logpRelax = np.loadtxt('protonCoolingTimes.dat',unpack=True,skiprows=1)
...
with PdfPages('electronCoolingTimes.pdf') as pdf:
    for r1 in range(np.size(r)//nE):
        ...
        ax1.plot(log_ge[r1*nE:(r1+1)*nE],logDiffKe[r1*nE:(r1+1)*nE],lw=1.5,ls='dotted',c='orange',label='Diff K')
        ax1.plot(log_ge[r1*nE:(r1+1)*nE],logDiffBe[r1*nE:(r1+1)*nE],lw=1.5,ls='dotted',label='Diff B')
        ax1.plot(log_ge[r1*nE:(r1+1)*nE],logDiffPare[r1*nE:(r1+1)*nE],lw=1.5,ls='dotted',label='Diff par.')
        ax1.plot(log_ge[r1*nE:(r1+1)*nE],logDiffPere[r1*nE:(r1+1)*nE],lw=1.5,ls='dotted',label='Diff perp.')
        ...
        ax1.plot(log_ge[r1*nE:(r1+1)*nE],logeRelax[r1*nE:(r1+1)*nE],lw=4,ls='solid',c='b',label='Relax')
...
with PdfPages('protonCoolingTimes.pdf') as pdf:
    for r1 in range(np.size(r)//nE):
        ...
        ax1.plot(log_gp[r1*nE:(r1+1)*nE],logDiffKp[r1*nE:(r1+1)*nE],lw=1.5,ls='dotted',c='orange',label='Diff K')
        ax1.plot(log_gp[r1*nE:(r1+1)*nE],logDiffBp[r1*nE:(r1+1)*nE],lw=1.5,ls='dotted',label='Diff B')
        ax1.plot(log_gp[r1*nE:(r1+1)*nE],logDiffParp[r1*nE:(r1+1)*nE],lw=1.5,ls='dotted',label='Diff par')
        ax1.plot(log_gp[r1*nE:(r1+1)*nE],logDiffPerp[r1*nE:(r1+1)*nE],lw=1.5,ls='dotted',label='Diff perp.')
        ...
        ax1.plot(log_gp[r1*nE:(r1+1)*nE],logpRelax[r1*nE:(r1+1)*nE],lw=4,ls='solid',c='b',label='Relax')
        ax1.axvline(logEmaxHe[0])
        ax1.legend(loc='best',fontsize=8)
        plt.tight_layout()
        pdf.savefig(fig)
